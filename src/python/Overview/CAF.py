from Monitoring.Overview.GUI import (
    ImageScraperSource,
    CompWorkspace,
    lemonimg,
    lrfimg,
...
class CAFDataProvider(Thread):
    ...
    def refresh(self):
        ...
        try:
            newdata = json.loads(request.urlopen(self.DATAURL).read())
        except:
            pass

        try:
            newtotal = request.urlopen(self.DISKURL).readlines()
        except:
            pass
...
class CAFDiskSource:
    ...
    def getfrac(self, label):
        ...
        return "text/plain", "%s %% (%.2f TB)" % (
            (total and ("%.2f" % (100.0 * size / total))) or "?.??",
            size,
        )
    ...
    def plot(self, group, *args, **kwargs):
        if group == "alcabydate":
            return self.getdata("alca", "ctime")
        elif group == "alcabysize":
            return self.getdata("alca", "size")
        elif group == "commbydate":
            return self.getdata("comm", "ctime")
        elif group == "commbysize":
            return self.getdata("comm", "size")
        elif group == "physbydate":
            return self.getdata("phys", "ctime")
        elif group == "physbysize":
            return self.getdata("phys", "size")
        elif group == "otherbydate":
            return self.getdata("other", "ctime")
        elif group == "otherbysize":
            return self.getdata("other", "size")
        elif group == "alcapercent":
            return self.getfrac("alca")
        elif group == "commpercent":
            return self.getfrac("comm")
        elif group == "physpercent":
            return self.getfrac("phys")
        elif group == "otherpercent":
            return self.getfrac("other")
        else:
            return None, None
