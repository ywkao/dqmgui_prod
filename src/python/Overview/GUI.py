...
def lemonimg(prov, kind, images, match, **kwargs):
    return {
        "html": mklnk(
            "http://lemonweb.cern.ch/lemon-%s/%s.php?" % (prov, kind),
            type="host",
            cluster=1,
            **kwargs
        ),
...
def lrfimg(images, match, **kwargs):
    return {
        "html": mklnk("http://lsf-rrd.cern.ch/lrf-lsf/info.php?", **kwargs),
        "match": match,
        "images": images,
    }
...
def slsimg(images, match, **kwargs):
    return {
        "html": mklnk("http://sls.cern.ch/sls/history.php?", **kwargs),
        "match": match,
        "images": images,
    }
...
class CompWorkspace:
    ...
    def _dostate(self, session, helptopic=None):
        ...
        return (
            ...
            view.name,
            ",".join(
                "{'title': '%s', 'label':'%s'}" % (x.name, x.label) for x in self.views
            ),
            session["comp.menu"],
    ...
    def _set(self, session, submenu=None, view=None, **kwargs):
        ...
        if view != None:
            if not isinstance(view, str) or view.lower() not in (
                x.label for x in self.views
            ):
    ...
    def run(self):
        while True:
            ...
            for s in self.infosrc:
                ...
                imgrefs = {}
                try:
                    data = request.urlopen(s["html"]).read().replace("\n", " ")
                    for imgtag in re.findall(rximg, data):
                        while True:
                            m = re.match(
                                "^([-A-Za-z0-9]+)=(\"[^\"]*\"|'[^']*'|\S+)\s*/?", imgtag
                            )
                            if not m:
                                break
                ...
                for name, url in imgrefs.items():
                    try:
                        ...
                        _logwarn(
                            "failed to fetch image %s from '%s': %s"
                            % (name, imgurl, str(e))
                        )
                        imgdata[name] = None
