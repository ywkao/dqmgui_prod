    ...
    def _set(
        ...
        **kwargs
        ...
        if sampledynsearch != None:
            if not isinstance(sampledynsearch, str) or sampledynsearch not in (
                "yes",
                "no",
            ):
        ...
        if sampletype != None:
            if not isinstance(sampletype, str):
                ...
                session["dqm.sample.type"] = int(
                    getattr(Accelerator.sampletype, sampletype)
                )
            else:
                raise HTTPError(500, "Incorrect sample type parameter")
        ...
        if filter != None:
            if not isinstance(filter, str) or filter not in (
                "all",
                "alarms",
                "nonalarms",
            ):
        ...
        if referenceshow != None:
            if not isinstance(referenceshow, str) or referenceshow not in (
                "all",
                "none",
                "customise",
            ):
        ...
        if referencepos != None:
            if not isinstance(referencepos, str) or referencepos not in (
                "overlay",
                "ratiooverlay",
                "on-side",
                "stacked",
        ...
        for refidx, refobj in (
            ...
            if refobj != None:
                ...
                m = re.match(
                    r"^other:(\d*):([-/A-Za-z0-9_]*)(:([A-Za-z0-9 ]*))?:([0-9.]*)$",
                    refobj,
                )
        ...
        if striptype != None:
            if not isinstance(striptype, str) or striptype not in (
                "object",
                "num-entries",
                "num-bins",
                "num-bytes",
        ...
        if submenu != None:
            if not isinstance(submenu, str) or submenu not in (
                "data",
                "service",
                "workspace",
            ):
        ...
        if size != None:
            if not isinstance(size, str) or size not in ("XS", "S", "M", "L", "XL"):
                raise HTTPError(500, "Incorrect size parameter")
            session["dqm.size"] = size

        if playprevws != None:
            if not isinstance(playprevws, str) or playprevws not in (
                x.name for x in self.gui.workspaces
            ):
        ...
        if playinterval != None:
            if not isinstance(playinterval, str) or not re.match(
                r"^\d+$", playinterval
            ):
        ...
        if playpos != None:
            if not isinstance(playpos, str) or not re.match(r"^\d+$", playpos):
