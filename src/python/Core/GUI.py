...
class Server:
    ...
    def __init__(self, cfgfile, cfg, modules):
        ...
        self.sources = [
            extension(modules, s[0] + "Source", self, cfg.serverDir + "/" + s[1], *s[2])
            for s in cfg.sources
        ]
        self.workspaces = [
            extension(modules, w[0] + "Workspace", self, *w[1]) for w in cfg.workspaces
        ]
        ...
        for name, m in sys.modules.items():
            if (
                ...
                if not processed:
                    self._addChecksum(
                        name,
                        inspect.getsourcefile(m) or inspect.getabsfile(m) or name,
                        inspect.getsource(m),
                    )
    ...
    def _templatePage(self, name, variables):
        ...
        params = {
            "CSS": "".join(x[1] for x in self.css),
    ...
    def _addCSSFragment(self, filename):
        ...
        if not filename in dict(self.css):
            ...
            if filename.startswith(self._yui):
                ...
                text = re.sub(
                    r"url\((\.\./)+([-a-z._/]+)\)",
                    r"url(%s/yui/\2)" % self.baseUrl,
                    re.sub(
                        r"url\(([-a-z._]+)\)",
                        r"url(%s/yui/%s/\1)" % (self.baseUrl, path),
                        text,
                    ),
                )
            if self._extjs and filename.startswith(self._extjs):
                ...
                text = re.sub(
                    r"url\((\.\./)+([-a-z._/]+)\)",
                    r"url(%s/extjs/resources/\2)" % self.baseUrl,
                    re.sub(
                        r"url\(([-a-z._]+)\)",
                        r"url(%s/extjs/resources/%s/\1)" % (self.baseUrl, path),
                        text,
                    ),
                )
            clean = re.sub(
    ...
    def _saveSession(self, session):
        ...
        self.lock.release()
        self.sessionthread.save(
            dict((k, deepcopy(session[k])) for k in session.keys() if k != "core.lock")
        )
    ...
    def jsrootfairy(self, *args, **kwargs):
        try:
            ...
            log(
                "WARNING: unable to produce JSROOT json: "
                + (str(e) + "\n" + o.getvalue()).replace("\n", " ~~ "),
    ...
    def jsonfairy(self, *args, **kwargs):
        ...
        try:
            if len(args) >= 1:
                for s in self.sources:
                    if getattr(s, "jsonhook", None) == args[0]:
                        ...
                        if kwargs.get("formatted") == "true":
                            ...
                            variables = {
                                "TITLE": "JSON represetation of histogram",
                                "JSON": data,
                            }
                ...
                # if not found any...
                return (
                    "JSON format of " + args[0] + " source plot is not supported yet."
                )
        except Exception as e:
            o = StringIO()
            traceback.print_exc(file=o)
            log(
                "WARNING: unable to produce a json: "
                + (str(e) + "\n" + o.getvalue()).replace("\n", " ~~ "),
                severity=logging.WARNING,
            )
    ...
    def plotfairy(self, *args, **kwargs):
        ...
        try:
            ...
            traceback.print_exc(file=o)
            log(
                "WARNING: unable to produce a plot: "
                + (str(e) + "\n" + o.getvalue()).replace("\n", " ~~ "),
                severity=logging.WARNING,
            )
        ...
        self._noResponseCaching()
        return serve_file(
            self.contentpath + "/images/missing.png", content_type="image/png"
        )
