...
#define DEBUG(x, msg)                                                          \
  if (debug >= x)                                                              \
  std::cout << "DEBUG[" << x << "]: " << msg << std::flush
...
public:
  OrderByNameIndex(std::vector<MonitorElementInfo> &minfo) : minfo_(minfo) {}
...
Regexp
    rxonline("^(?:.*/)?DQM_V\\d+(?:_[A-Za-z0-9]+)?_R(\\d+)\\.(dat|pb|root)$");
...
Regexp rxoffline(
    "^(?:.*/)?DQM_V\\d+_R(\\d+)((?:__[-A-Za-z0-9_]+){3})\\.(dat|pb|root)$");
...
/// string.
Regexp rxrelvalrundepmc(
    "^/RelVal[^/]+/(CMSSW(?:_[0-9]+)+(?:_pre[0-9]+)?)[-_].*rundepMC.*$");
...
static inline uint32_t myroundup(uint32_t value, uint32_t unit) {
...
static inline void writeDouble(std::ofstream &iwrite, const char *prefix,
                               double val) {
  char buf[64];
...
void CompressMessage(const OriginalMessage &source, CompressionFactor k,
...
static MEClass classifyMonitorElement(DQMStore & /* store */,
                                      MonitorElement &obj,
  ...
  if (name.compare(0, 4, "Run ") == 0 &&
    ...
    if (category == "Run summary")
      ...
      if (errno != 0 || !end || *end != '-' || !isdigit((unsigned)end[1]))
        return ME_CLASS_BAD;
  ...
  case MonitorElement::DQM_KIND_STRING:
    ...
    if (info.name.size() > 23 &&
        info.name.compare(info.name.size() - 23, 23,
                          "/EventInfo/iLumiSection") == 0)
      meta.numLumiSections =
          std::max(meta.numLumiSections, uint64_t(atoll(info.data.c_str())));
    else if (info.name.size() > 27 &&
             info.name.compare(info.name.size() - 27, 27,
                               "/EventInfo/processTimeStamp") == 0)
      meta.processedTime =
          std::max(meta.processedTime, uint64_t(atof(info.data.c_str()) * 1e9));
    else if (info.name.size() > 26 &&
             info.name.compare(info.name.size() - 26, 26,
                               "/EventInfo/processedEvents") == 0)
      meta.numEvents =
          std::max(meta.numEvents, uint64_t(atoll(info.data.c_str())));
    else if (info.name.size() > 28 &&
             info.name.compare(info.name.size() - 28, 28,
                               "/EventInfo/runStartTimeStamp") == 0)
      meta.runStartTime =
          std::max(meta.runStartTime, uint64_t(atof(info.data.c_str()) * 1e9));
    break;
...
static void *classifyMonitorElementRange(void *arg) {
  MEClassifyTask &task = *(MEClassifyTask *)arg;
  DEBUG(1, "classifying range ["
               << task.begin << ", " << task.end << ") in thread " << std::hex
               << (unsigned long)pthread_self() << std::dec << "\n");

  for (size_t m = task.begin, e = task.end; m < e; ++m) {
    MonitorElementInfo info;
    switch (classifyMonitorElement(task.store, *task.mes[m], info, task.meta,
                                   *task.fi.sample)) {
    case ME_CLASS_BAD:
      task.nerrors++;
      std::cerr << task.fi.path.name() << ": " << task.mes[m]->getFullname()
                << ": warning: monitor element not recognised\n";
      continue;

    case ME_CLASS_SKIP:
      task.nerrors++;
      DEBUG(3, "skipping monitor element '" << task.mes[m]->getFullname()
                                            << "'\n");
      continue;

    case ME_CLASS_KEEP:
      DEBUG(3, ""
                   << "flags=" << std::hex << info.flags << std::dec << " tag="
                   << info.tag << " object[0]=" << (void *)info.object[0]
                   << " object[1]=" << (void *)info.object[1] << " category="
                   << info.category << " lumi=" << info.lumibegin << ':'
                   << info.lumiend << " name='" << info.name << "' data='"
                   << info.data << "' qreports='" << info.qreports
                   << "' nentries=" << std::setprecision(DBL_DIG + 2)
                   << info.nentries << " nbins=" << info.nbins[0] << "/"
                   << info.nbins[1] << "/" << info.nbins[2]
                   << " mean=" << info.mean[0] << "/" << info.mean[1] << "/"
                   << info.mean[2] << " rms=" << info.rms[0] << "/"
                   << info.rms[1] << "/" << info.rms[2] << " bounds=["
                   << info.bounds[0][0] << ":" << info.bounds[0][1] << ", "
...
static bool extractRunNumber(FileInfo &fi, SampleInfo &si, RegexpMatch &m,
                             int matchnr) {
  char *end = 0;
  ...
  if (err > 0) {
    std::cerr << fi.path.name()
              << ": error extracting run number: " << strerror(err)
              << " (error code " << err << ")\n";
    return false;
...
static bool fileInfoFromName(FileInfo &fi, SampleInfo &si) {
  RegexpMatch m;
  ...
  // and classify the file to "data", "relval" or "monte carlo".
    ...
    si.dataset =
        StringOps::replace(m.matchString(fi.path.name(), 2).c_str(), "__", "/");
...
static bool verifyFileInfo(const FileInfo &fi) {
  RegexpMatch m;
  ...
  if (!rxdataset.exactMatch(si.dataset)) {
    std::cerr << fi.path.name() << ": dataset '" << si.dataset
              << "' not valid\n";
    return false;
  }
  ...
  case TYPE_RELVAL_RUNDEPMC:
    ...
    if (si.runnr != 1 && !rxrelvalrundepmc.match(si.dataset, 0, 0, &m)) {
      std::cerr << fi.path.name()
                ...
                << si.runnr
                << ") and sample is not a valid Run Dependent MC.\n";
      return false;
    }
...
static int initIndex(const Filename &indexdir) {
...
static void contract(VisDQMIndex &ix, VisDQMIndex::Sample &s, uint64_t nsample,
  ...
  for (int kind = 1; kind >= 0; --kind) {
    VisDQMFile *rfile =
        ix.open(VisDQMIndex::MASTER_FILE_INFO + kind, s.files[kind] >> 16,
                s.files[kind] & 0xffff, VisDQMFile::OPEN_READ);
    VisDQMFile *wfile = ix.open(
        VisDQMIndex::MASTER_FILE_INFO + kind, s.files[kind] >> 16,
        (s.files[kind] & 0xffff) + (rfile ? 1 : 0), VisDQMFile::OPEN_WRITE);

    DEBUG(1, "writing out new data file "
                 << kind << ": in [" << (s.files[kind] >> 16) << ':'
                 << (s.files[kind] & 0xffff)
                 << "]=" << (rfile ? rfile->path().name() : "(none)") << " out "
                 << wfile->path() << '\n');
...
static void extend(VisDQMIndex &ix, VisDQMIndex::Sample &s, uint64_t nsample,
                   std::vector<MonitorElementInfo> &minfo,
                   StringAtomTree &objnames, std::list<Filename> &oldfiles,
                   std::list<Filename> &newfiles, bool generic_stream_file,
  ...
  for (size_t i = 0, e = minfo.size(); i < e; ++i) {
    IndexKey tmp(nsample, minfo[i].category, minfo[i].lumiend,
  ...
  for (int kind = 1; kind >= 0; --kind) {
    ...
    while (ix < minfo.size()) {
      ...
      DEBUG(2, "transferring previous keys up to " << ix << ':' << std::hex
                                                   << minfo[minfoix[ix]].nameidx
                                                   << std::dec << '\n');
      wrhead.xfer(rdhead, minfo[minfoix[ix]].nameidx, &rkey, &rstart, &rend);

      // Now write new keys, or replace old objects.
      while (ix < minfo.size() &&
             (rdhead.isdone() || rkey >= minfo[minfoix[ix]].nameidx)) {
        ...
        if (kind == 1) {
          if (!generic_stream_file) {
            ...
            if (!(info.object[0] || info.object[1]))
              ...
              DEBUG(3, (rkey == info.nameidx ? "updating" : "inserting")
                           << " data for " << ix << ':' << std::hex
                           << info.nameidx << std::dec << " ('" << info.name
                           << "'): " << info.ndata << " bytes\n");
          ...
          DEBUG(3, (rkey == info.nameidx ? "updating" : "inserting")
                       << " summary for " << ix << ':' << std::hex
                       << info.nameidx << std::dec << " ('" << info.name
                       << "'): " << sizeof(VisDQMIndex::Summary)
                       << " summary + " << datalen << " data + " << qreplen
                       << " qreports bytes\n");
          wrhead.allocate(
              info.nameidx,
    ...
    if (rfile) {
      ...
      s.files[kind] =
          (s.files[kind] & 0xffff0000) | ((s.files[kind] + 1) & 0x0000ffff);
    }
...
static void readFileStream(FileInfo &fi, std::string &streamerinfo,
                           size_t &numObjs, uint64_t &numEvents,
                           uint64_t &numLumiSections, uint64_t &processedTime,
  ...
  if (!iread)
    throw VisDQMError(
        0, fi.path.name(),
        StringFormat("failed to read file #%1").arg(fi.path.name()));
  iread >> sr.runNumber >> sr.numObjects >> numEvents >> numLumiSections >>
      runStartTime >> processedTime >> dummy;
  ...
  while (true) {
    ...
    if (cur.name == MEROOTBOUNDARY)
      break;
    iread >> cur.category >> cur.lumibegin >> cur.flags >> datalen >> qreplen >>
        cur.ndata >> cur.tag >> dummy;
  ...
  for (; mi != me; ++mi) {
    ...
    case MonitorElement::DQM_KIND_TPROFILE2D:
      /* This assert was used to debug the .dat format.
         iread >> key;
         std::cout << "key: " << key << " nameidx: " << mi->nameidx << " flags:
         " << mi->flags << std::endl; assert(key == mi->nameidx);
      */
      iread >> dummy;
...
/** Read a file streamed out of the index. */
static void readFileStreamProtocolBuffer(
    FileInfo &fi, std::string &streamerinfo, size_t &numObjs,
    uint64_t &numEvents, uint64_t &numLumiSections, uint64_t &processedTime,
    uint64_t &runStartTime, std::vector<MonitorElementInfo> &minfo,
    StringAtomTree &rootobjs) {
  // Now scan the file.
  SampleInfo &si = *fi.sample;
  ...
  dqmgui::StreamSample::Header *header = new dqmgui::StreamSample::Header();
  ...
  std::vector<MonitorElementInfo>::iterator mi = minfo.begin();
  std::vector<MonitorElementInfo>::iterator me = minfo.end();
  dqmgui::StreamSample::MERoot *me_root = new dqmgui::StreamSample::MERoot();
  for (int j = 0; mi != me; ++mi) {
    ...
    case MonitorElement::DQM_KIND_TPROFILE2D:
      me_root->Clear();
      const dqmgui::StreamSample::GenericCompressed &me_root_c =
          sample.me_root_c(j);
...
static int addFiles(const Filename &indexdir, std::list<FileInfo> &files) {
  ...
  while (!files.empty()) {
    ...
    DEBUG(1,
          "importing " << fi.path.name() << ": sample [#" << si.index
                       << ", type '"
                       << (si.type == TYPE_DATA              ? "data"
                           : si.type == TYPE_RELVAL          ? "relval"
                           : si.type == TYPE_MC              ? "mc"
                           : si.type == TYPE_RELVAL_RUNDEPMC ? "relval_rundepmc"
                           : si.type == TYPE_RUNDEPMC        ? "rundep mc"
                                                             : "other")
                       << "', runnr " << si.runnr << ", version '" << si.version
                       << "', dataset '" << si.dataset << "']\n");
    ...
    try {
      ...
      if (!streamFile && !streampbFile) {
        ...
        numObjs = mes.size();
        MEClassifyTask tasks[NUM_TASKS] = {
            MEClassifyTask(fi, store, mes), MEClassifyTask(fi, store, mes),
            MEClassifyTask(fi, store, mes), MEClassifyTask(fi, store, mes)};
        ...
        for (size_t i = 0; i < NUM_TASKS; ++i) {
          ...
          tasks[i].nerrors = 0;
          if (pthread_create(&tasks[i].thread, 0, &classifyMonitorElementRange,
                             &tasks[i]))
            throw VisDQMError(
                0, fi.path.name(),
        ...
        for (size_t i = 0; i < NUM_TASKS; ++i) {
          ...
          tasks[0].meta.numEvents =
              std::max(tasks[0].meta.numEvents, tasks[i].meta.numEvents);
          tasks[0].meta.numLumiSections = std::max(
              tasks[0].meta.numLumiSections, tasks[i].meta.numLumiSections);
          tasks[0].meta.processedTime = std::max(tasks[0].meta.processedTime,
                                                 tasks[i].meta.processedTime);
          tasks[0].meta.runStartTime =
              std::max(tasks[0].meta.runStartTime, tasks[i].meta.runStartTime);

          minfo.insert(minfo.end(), tasks[i].minfo.begin(),
                       tasks[i].minfo.end());
        ...
        if (streamFile)
          readFileStream(fi, streamerinfoFromStream, numObjs, numEvents,
                         numLumiSections, processedTime, runStartTime, minfo,
                         rootobjs);
        if (streampbFile)
          readFileStreamProtocolBuffer(
              fi, streamerinfoFromStream, numObjs, numEvents, numLumiSections,
              processedTime, runStartTime, minfo, rootobjs);
      }
      ...
      readStrings(pathnames, master,
                  IndexKey(0, VisDQMIndex::MASTER_SOURCE_PATHNAME));
      readStrings(dsnames, master,
                  IndexKey(0, VisDQMIndex::MASTER_DATASET_NAME));
      readStrings(vnames, master,
                  IndexKey(0, VisDQMIndex::MASTER_CMSSW_VERSION));
      readStrings(objnames, master,
                  IndexKey(0, VisDQMIndex::MASTER_OBJECT_NAME));
      readStrings(streamers, master,
      ...
      for (VisDQMFile::ReadHead rdhead(
        ...
        DEBUG(2, "considering sample #" << (samples.size() - 1) << ": dataset '"
                                        << dsnames.key(s.datasetNameIdx)
                                        << "', version='"
                                        << vnames.key(s.cmsswVersion).c_str()
                                        << "', runnr=" << s.runNumber << "\n");
        ...
        if (!found) {
          ...
          found =
              (s.datasetNameIdx == sadataset.index() &&
               s.cmsswVersion == saversion.index() && s.runNumber == si.runnr);
      ...
      if (!found) {
        ...
        DEBUG(2, "adding new sample #"
                     << (samples.size() - 1) << ", info-file #"
                     << (datafile[0] >> 16) << ':' << (datafile[0] & 0xffff)
        ...
        extend(ix, s, samples.size() - 1, minfo, objnames, oldfiles, newfiles,
      ...
      writeStrings(wrhead, pathnames,
                   IndexKey(0, VisDQMIndex::MASTER_SOURCE_PATHNAME));

      DEBUG(1, "adding " << (dsnames.size() - 1) << " dataset names\n");
      writeStrings(wrhead, dsnames,
                   IndexKey(0, VisDQMIndex::MASTER_DATASET_NAME));

      DEBUG(1, "adding " << (vnames.size() - 1) << " version names\n");
      writeStrings(wrhead, vnames,
                   IndexKey(0, VisDQMIndex::MASTER_CMSSW_VERSION), 0);

      DEBUG(1, "adding " << (objnames.size() - 1) << " object names\n");
      writeStrings(wrhead, objnames,
                   IndexKey(0, VisDQMIndex::MASTER_OBJECT_NAME));

      DEBUG(1, "adding " << (streamers.size() - 1) << " streamers\n");
      writeStrings(wrhead, streamers,
                   IndexKey(0, VisDQMIndex::MASTER_TSTREAMERINFO));
    ...
    // cancel index transaction, and quit.
      ...
      std::cerr << fi.path.name() << ": error reading file: " << err.what()
                << '\n';
...
static int removeFiles(const Filename &indexdir, const std::string &dataset,
                       int32_t runnr) {
  ...
  try {
    ...
    for (VisDQMFile::ReadHead rdhead(
      ...
      if (!found) {
        // Check for a match with our dataset spec.
        found = (s.datasetNameIdx == sadataset.index() && s.runNumber == runnr);
    ...
    DEBUG(1, "adding " << (vnames.size() - 1) << " version names\n");
    writeStrings(wrhead, vnames, IndexKey(0, VisDQMIndex::MASTER_CMSSW_VERSION),
                 0);
...
// ----------------------------------------------------------------------
/** Add monitor element data to sample data files. */
static void remapdata(VisDQMIndex &ix, VisDQMIndex &other,
                      VisDQMIndex::Sample &s, uint64_t origsample,
                      uint64_t newsample,
                      const std::vector<size_t> &objnames2ix,
                      uint32_t *datafile, VisDQMFile *wfile[],
                      std::list<Filename> &oldfiles,
                      std::list<Filename> &newfiles) {
  // Copy data to summary and data files, keeping data in key order.
  ...
  for (int kind = 1; kind >= 0; --kind) {
    VisDQMFile *rfile =
        other.open(VisDQMIndex::MASTER_FILE_INFO + kind, s.files[kind] >> 16,
                   s.files[kind] & 0xffff, VisDQMFile::OPEN_READ);
    ...
    if (wfile[kind] == 0) {
      oldfile =
          ix.open(VisDQMIndex::MASTER_FILE_INFO + kind, datafile[kind] >> 16,
                  datafile[kind] & 0xffff, VisDQMFile::OPEN_READ);

      // Transfer any original file contents intact.
      if (oldfile) {
        if (ix.size(kind, datafile[kind] >> 16, datafile[kind] & 0xffff) <
            500000000) {
          ...
          wfile[kind] = ix.open(VisDQMIndex::MASTER_FILE_INFO + kind,
                                datafile[kind] >> 16, (datafile[kind] & 0xffff),
      ...
      if (wfile[kind] == 0) {
        ...
        wfile[kind] =
            ix.open(VisDQMIndex::MASTER_FILE_INFO + kind, datafile[kind] >> 16,
                    (datafile[kind] & 0xffff), VisDQMFile::OPEN_WRITE);
      }
    } else {
      // Start a new data file if the last used file is too large.
      if (ix.size(kind, datafile[kind] >> 16, datafile[kind] & 0xffff) >
    ...
    DEBUG(1, "appending new data file "
                 << kind << ": in [" << (s.files[kind] >> 16) << ':'
                 << (s.files[kind] & 0xffff) << "]=" << rfile->path().name()
                 << " out " << wfile[kind]->path() << ", replaces "
                 << (oldfile ? oldfile->path().name() : "(none)") << '\n');

    VisDQMFile::WriteHead wrhead(wfile[kind]);

    // Build remap table of objects to new object index.
    // We don't read the actual objects yet.
    DEBUG(2, "remapping objects starting at " << std::hex << begin << std::dec
                                              << "\n");
    for (VisDQMFile::ReadHead rdhead(rfile, begin); !rdhead.isdone();
         rdhead.next()) {
      ...
      IndexKey wkey(newsample, rkey.type(), rkey.lumiend(),
                    objnames2ix[rkey.objnameidx()]);

      DEBUG(2, "mapping object key " << std::hex << rkey << " to key " << wkey
                                     << std::dec << '\n');
    ...
    for (size_t i = 0, e = remapping.size(); i != e; ++i) {
      ...
      DEBUG(2, "copying object key " << std::hex << rkey << " to key "
                                     << remapping[i].first << std::dec << ", "
                                     << size << " bytes\n");
...
static int mergeIndexes(const Filename &indexdir,
  ...
  while (!mergeix.empty()) {
    ...
    try {
      ...
      readStrings(othpathnames, othermaster,
                  IndexKey(0, VisDQMIndex::MASTER_SOURCE_PATHNAME));
      readStrings(othdsnames, othermaster,
                  IndexKey(0, VisDQMIndex::MASTER_DATASET_NAME));
      readStrings(othvnames, othermaster,
                  IndexKey(0, VisDQMIndex::MASTER_CMSSW_VERSION));
      readStrings(othobjnames, othermaster,
                  IndexKey(0, VisDQMIndex::MASTER_OBJECT_NAME));
      readStrings(othstreamers, othermaster,
                  IndexKey(0, VisDQMIndex::MASTER_TSTREAMERINFO));
      ...
      for (VisDQMFile::ReadHead rdhead(
        ...
        for (size_t i = 0, e = samples.size(); i != e; ++i)
          for (int j = 0; j < 2; ++j)
            if ((samples[i].files[j] & 0xffff0000) ==
                (datafile[j] & 0xffff0000))
              samples[i].files[j] = datafile[j];
      }
    ...
    // cancel index transaction, and quit.
      ...
      std::list<Filename>::iterator i, e;
      std::cerr << mergeix.front().name()
                << ": error reading index: " << err.what() << '\n';
      for (i = newfiles.begin(), e = newfiles.end(); i != e; ++i) {
        DEBUG(1, "undoing new file " << i->name() << std::endl);
        Filename::remove(*i, false, true);
      }
...
static int fixStreamerInfo(const Filename &indexdir, size_t streamerid) {
  // Grab streamer info before we've opened any ROOT files.
  ...
  try {
    ...
    for (size_t i = 1, e = streamers.size(); i != e; ++i) {
      ...
      if (i == streamerid) {
        ASSERT(extended_streamers.search(partial_streamerinfoFromRoot) !=
               StringAtomTree::npos);
        extended_streamers.insert(partial_streamerinfoFromRoot);
        DEBUG(2, partial_streamerinfoFromRoot.size()
                     << " bytes of new streamerInfo will replace "
                     << original.size()
    ...
    DEBUG(1, "adding " << (streamers.size() - 1) << " streamers\n");
    writeStrings(wrhead, extended_streamers,
                 IndexKey(0, VisDQMIndex::MASTER_TSTREAMERINFO));
...
static int dumpIndex(const Filename &indexdir, DumpType what, size_t sampleid) {
  VisDQMFile *master;
  VisDQMIndex ix(indexdir);
  ...
  ix.beginRead(master);
  for (VisDQMFile::ReadHead rdhead(master, IndexKey(0, 0)); !rdhead.isdone();
       rdhead.next()) {
    void *begin;
    void *end;
    IndexKey key;
  ...
  if (what == DUMP_ALL || what == DUMP_CATALOGUE) {
    ...
    for (size_t n = 0; si != se; ++si, ++n)
      if (n == sampleid || sampleid == ALL_SAMPLES)
        std::cout << "SAMPLE #" << n << " first-import=" << si->firstImportTime
                  << " last-import=" << si->lastImportTime
                  << " import-version=" << si->importVersion << " info-file=#"
                  ...
                  << pathnames.key(si->sourceFileIdx) << " dataset-name=#"
                  << si->datasetNameIdx << '/'
                  << dsnames.key(si->datasetNameIdx) << " streamer-info=#"
                  << si->streamerInfoIdx << '/'
                  << streamers.key(si->streamerInfoIdx).size()
                  << " runnr=" << si->runNumber << " cmssw-version='"
                  << vnames.key(si->cmsswVersion).c_str()
    ...
    for (size_t i = 1, e = pathnames.size(); i != e; ++i)
      std::cout << "SOURCE-FILE #" << i << "='" << pathnames.key(i) << "'\n";

    for (size_t i = 1, e = dsnames.size(); i != e; ++i)
      std::cout << "DATASET-NAME #" << i << "='" << dsnames.key(i) << "'\n";

    for (size_t i = 1, e = vnames.size(); i != e; ++i)
      std::cout << "CMSSW-VERSION #" << i << "='" << vnames.key(i) << "'\n";

    for (size_t i = 1, e = objnames.size(); i != e; ++i)
      std::cout << "OBJECT-NAME #" << i << "='" << objnames.key(i) << "'\n";

    for (size_t i = 1, e = streamers.size(); i != e; ++i) {
      MD5Digest md5;
      const std::string &data = streamers.key(i);
      md5.update(&data[0], data.size());
      std::cout << "STREAMER #" << i << "=[len:" << data.size()
                << ", md5:" << md5.format() << "]\n";
    }
  }
  ...
  if (what == DUMP_ALL || what == DUMP_INFO) {
    ...
    for (size_t n = 0; si != se; ++si, ++n) {
      ...
      if (VisDQMFile *f =
        ...
        for (VisDQMFile::ReadHead rdhead(f, sample); !rdhead.isdone();
             rdhead.next()) {
          IndexKey key;
          void *begin;
          void *end;
          ...
          if (key.sampleidx() == n) {
            ...
            uint32_t report =
                s->properties & VisDQMIndex::SUMMARY_PROP_REPORT_MASK;
            const char *data = (s->dataLength ? (const char *)(s + 1) : "");
            const char *qdata =
                (s->qtestLength ? ((const char *)(s + 1) + s->dataLength) : "");

            std::cout
                << "ME [sample:" << key.sampleidx()
                << ", category:" << key.type() << ", lumiend:" << key.lumiend()
                << ", nameidx:" << key.objnameidx() << "] name='"
                << objnames.key(key.objnameidx()) << "' properties=" << std::hex
                << s->properties << std::dec << "["
                ...
                << (type == VisDQMIndex::SUMMARY_PROP_TYPE_TPROF2D ? ", TPROF2D"
                                                                   : "")
                << ((report & VisDQMIndex::SUMMARY_PROP_REPORT_ALARM)
                        ? ", ALARM"
                        : "")
                << ((report & VisDQMIndex::SUMMARY_PROP_REPORT_ERROR)
                        ? ", ERROR"
                        : "")
                << ((report & VisDQMIndex::SUMMARY_PROP_REPORT_WARN) ? ", WARN"
                                                                     : "")
                << ((report & VisDQMIndex::SUMMARY_PROP_REPORT_OTHER)
                        ? ", OTHER"
                        : "")
                << ((s->properties & VisDQMIndex::SUMMARY_PROP_EFFICIENCY_PLOT)
                        ? ", EFF"
                        : "")
                ...
                << " object-length=" << s->objectLength << " tag=" << s->tag
                << " nentries=" << s->nentries
                << " axis-x=[nbins:" << s->nbins[0] << ", mean:" << s->mean[0]
                << ", rms:" << s->rms[0] << ", min:" << s->bounds[0][0]
                << ", max:" << s->bounds[0][1]
                << "] axis-y=[nbins:" << s->nbins[1] << ", mean:" << s->mean[1]
                << ", rms:" << s->rms[1] << ", min:" << s->bounds[1][0]
                << ", max:" << s->bounds[1][1]
                << "] axis-z=[nbins:" << s->nbins[2] << ", mean:" << s->mean[2]
                << ", rms:" << s->rms[2] << ", min:" << s->bounds[2][0]
                << ", max:" << s->bounds[2][1] << "] data='" << data
                << "' qreports=[";
            ...
            for (qi = qreports.begin(), qe = qreports.end(); qi != qe; ++qi) {
              ...
              std::cout << sep << "[status:" << qi->code << ", result:" << buf
                        << ", name:'" << qi->qtname << "', algorithm:'"
                        << qi->algorithm << "', message:'" << qi->message
                        << "']";
              sep = ", ";
            }
      ...
      if (VisDQMFile *f =
        ...
        for (VisDQMFile::ReadHead rdhead(f, sample); !rdhead.isdone();
          ...
          if (key.sampleidx() == n) {
            ...
            std::cout << "ME [sample:" << key.sampleidx()
                      << ", category:" << key.type()
                      << ", lumiend:" << key.lumiend()
                      << ", nameidx:" << key.objnameidx()
                      << "] data-length=" << len
                      << " object=[ptr:" << (void *)obj
...
static int streamout(const Filename &indexdir, size_t sampleid) {
  VisDQMFile *master;
  VisDQMIndex ix(indexdir);
  std::list<VisDQMIndex::Sample> samples;
  StringAtomTree pathnames(PATHNAMES);
  ...
  for (size_t n = 0; si != se; ++si, ++n) {
    ...
    if (!out)
      throw VisDQMError(0, fname,
                        StringFormat("failed to open file #%1").arg(fname));

    out << si->runNumber << " " << si->numObjects << " " << si->numEvents << " "
        << si->numLumiSections << " " << si->runStartTime << " "
        << si->processedTime << " "
    ...
    if (f) {
      ...
      for (VisDQMFile::ReadHead rdhead(f, sample); !rdhead.isdone();
        ...
        if (key.sampleidx() == n) {
          ...
          out << objnames.key(key.objnameidx()) << "\n"
              << key.type() << " " << key.lumiend() << " " << s->properties
              << " " << s->dataLength << " " << s->qtestLength << " "
              << s->objectLength << " " << s->tag << " " << s->nentries << " "
              << s->nbins[0];
    ...
    if (ff) {
      out << MEROOTBOUNDARY << "\n";
      IndexKey sample(n, 0, 0, 0);
      for (VisDQMFile::ReadHead rddata(ff, sample); !rddata.isdone();
           rddata.next()) {
        IndexKey key;
        void *begin;
        void *end;
        ...
        if (key.sampleidx() == n) {
          rawdata.clear();
          rawdata.insert(rawdata.end(), (unsigned char *)begin,
                         (unsigned char *)end);
          out << hexlify(rawdata);
          out << std::endl;
        } else
...
static int streamoutProtocolBuffer(const Filename &indexdir, size_t sampleid) {
  VisDQMFile *master;
  VisDQMIndex ix(indexdir);
  std::list<VisDQMIndex::Sample> samples;
  StringAtomTree pathnames(PATHNAMES);
  ...
  for (size_t n = 0; si != se; ++si, ++n) {
    ...
    dqmgui::StreamSample::GenericCompressed *header_c =
        sample.mutable_header_c();
    ...
    if (f) {
      ...
      for (VisDQMFile::ReadHead rdhead(f, curr_sample); !rdhead.isdone();
           rdhead.next()) {
        IndexKey key;
        void *begin;
        void *end;
    ...
    if (ff) {
      ...
      IndexKey curr_sample(n, 0, 0, 0);
      for (VisDQMFile::ReadHead rddata(ff, curr_sample); !rddata.isdone();
           rddata.next()) {
        IndexKey key;
        void *begin;
        void *end;
        ...
        rddata.get(&key, &begin, &end);
        if (key.sampleidx() == n) {
          me_root->set_rawdata((const void *)begin, (((unsigned char *)end) -
                                                     ((unsigned char *)begin)) *
                                                        sizeof(char));
          dqmgui::StreamSample::GenericCompressed *me_root_c =
              sample.add_me_root_c();
  ...
  int filedescriptor =
      open(fname.c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
  if (filedescriptor == -1)
    throw VisDQMError(0, fname,
                      StringFormat("failed to open file #%1").arg(fname));
  ...
  if (!sample.SerializeToZeroCopyStream(&gzip_stream))
    throw VisDQMError(0, fname,
                      StringFormat("failed to write file #%1").arg(fname));
...
static int showusage(void) {
  std::cerr
      << "Usage: " << app.name() << " [--[no-]debug] TASK OPTIONS\n\n  "
      << app.name() << " [OPTIONS] create INDEX-DIRECTORY\n  " << app.name()
      << " [OPTIONS] add [--dataset DATASET-NAME] INDEX-DIRECTORY [ROOT|DAT "
         "FILE...]\n  "
      << app.name()
      << " [OPTIONS] remove --dataset DATASET-NAME --run RUNNR "
         "INDEX-DIRECTORY\n  "
      << app.name()
      << " [OPTIONS] merge INDEX-DIRECTORY [IMPORT-INDEX-DIRECTORY...]\n  "
      << app.name()
      << " [OPTIONS] dump [--sample SAMPLE-ID] INDEX-DIRECTORY [{ catalogue | "
         "info | data | all }]\n  "
      << app.name()
      << " [OPTIONS] stream --sample SAMPLE-ID INDEX-DIRECTORY\n  "
      << app.name()
...
int main(int argc, char **argv) {
  ...
  if (task == TASK_ADD) {
    ...
    for (; arg < argc; ++arg) {
      if (!strcmp(argv[arg], "--sample")) {
        ...
        std::cerr << app.name()
                  << ": --sample option is mandatory when streaming a sample\n";
        return showusage();
      }
    }
  }
  ...
  if (task == TASK_CREATE) {
    ...
    if (arg != argc) {
      std::cerr << app.name()
                << ": too many arguments to 'fixstreamers' task\n";
      return showusage();
    }
    ...
    DEBUG(1, "index '" << indexdir.name() << "', dataset '" << dataset
                       << "', run " << runnr << "\n");
    ...
    DEBUG(1, "index '" << indexdir.name() << "', default dataset '" << dataset
                       << "'\n");
    ...
    for (; arg < argc; ++arg) {
      ...
      for (di = samples.begin(), de = samples.end(); di != de; ++di)
        if (di->type == si.type && di->runnr == si.runnr &&
            di->dataset == si.dataset && di->version == si.version) {
          fi.sample = &*di;
          break;
        }
    ...
    if (arg < argc) {
      if (!strcmp(argv[arg], "catalogue"))
        ...
        std::cerr << app.name() << ": unexpected dump option '" << argv[arg]
                  << "', expected one of index, info,"
                  << " data, all\n";
        return EXIT_FAILURE;
      }
