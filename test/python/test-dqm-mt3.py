...
class StripSource(Native.DQMStripChartSource):
    ...
    def plot(self, *path, **options):
        sources = dict(
            (s.plothook, s) for s in self.sources if getattr(s, "plothook", None)
        )
...
gui.sources.append(
    ArchiveSource(gui, {"index": "/build/lat/dqmgui/ix", "rxonline": "^/Global/"})
)
                ...
                print(
                    ...
                    len(
                        [
                            x
                            for x in data
                            if x["kind"] not in ("AutoUpdate", "DQMHeaderRow")
                        ]
                    ),
                )
                print(
